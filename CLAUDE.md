...
- `chart:lint`, `chart:template` and `chart:test` all depend on `chart:prepare-values`, which runs `.github/scripts/prepare-values.sh`. That script merges each chart's `ci/_common.sh` overrides straight into `charts/<chart>/values.yaml` via `sponge` — an in-place, destructive overwrite of the real file (e.g. it stomps `global.clusterName`/`monitoring.grafana.adminPassword` with CI test values), not a copy. It's meant for a disposable CI checkout. Running these tasks against a local working tree permanently mutates `values.yaml`; `git checkout -- charts/<chart>/values.yaml` afterwards to restore it. Prefer plain `helm lint`/`helm template` locally when just sanity-checking a values change.
