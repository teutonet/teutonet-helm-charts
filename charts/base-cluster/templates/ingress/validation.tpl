...
{{- else if eq .Values.ingress.provider "nginx" -}}
  {{- $existingTraefik := lookup "helm.toolkit.fluxcd.io/v2" "HelmRelease" "ingress" "traefik" -}}
  {{- if $existingTraefik -}}
    {{- fail "Cannot switch to nginx while traefik is installed. If you want to switch to nginx, please delete the HelmRelease 'ingress/traefik' first. Note: You might want to set .Values.ingress.IP to the current traefik LoadBalancer IP to keep the same IP. Warning: Switching providers will cause downtime until the new provider is fully deployed." -}}
  {{- end -}}
{{- end -}}

{{- if .Values.ingress.IP -}}
  {{- $loadBalancerIP := (list nil) | first -}}
  {{- $serviceName := (eq .Values.ingress.provider "traefik") | ternary "traefik" "ingress-nginx-controller" -}}
