...
{{- define "t8s-cluster.kubelet.featureGates"}}
  {{- include "t8s-cluster.featureGates.forComponent" (dict "component" "kubelet" "context" .context) -}}
{{- end -}}
...
{{- define "t8s-cluster.patches.kubelet.default" -}}
  ...
  {{- end -}}
  {{- $values = set $values "featureGates" (include "t8s-cluster.kubelet.featureGates" (dict "context" .context) | fromYaml) -}}
  {{- $patches := list -}}
...
{{- define "t8s-cluster.patches.kubelet.patches" -}}
  {{- $_ := mustMerge . (pick .context "Values") -}}
  {{- $patches := include "t8s-cluster.patches.kubelet.default" (dict "context" .context) | fromYamlArray -}}
